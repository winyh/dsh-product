...
function registerProductTools(ctx, config, fs, web) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			let value;
			try {
				value = JSON.parse(args.feedbackJson);
			} catch (error) {
				throw new Error(`feedbackJson must be valid JSON: ${error instanceof Error ? error.message : String(error)}`);
			}
			const source = typeof value === "object" && value !== null && "data" in value ? value.data : value;
			const review = reviewArtifact(source);
			const record = typeof source === "object" && source !== null ? source : {};
			const sourceType$1 = typeof record.artifactType === "string" ? record.artifactType : "unknown";
			const warnings = [...review.warnings];
			if (!["sales-feedback-handoff", "beta-feedback-import"].includes(sourceType$1)) warnings.push(`来源工件类型为 ${sourceType$1}，不是标准销售或 Beta 反馈。`);
			...
			return wrapResult({
				...
				...args.dueDate?.trim() ? { dueDate: args.dueDate.trim() } : {},
				...args.evidence?.trim() ? { evidence: args.evidence.trim() } : {},
				warnings,
				nextActions
			}, { nextActions });
		}

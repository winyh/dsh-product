...
//#region src/artifact-integrity.ts
...
const artifactDomain = "dsh-product";
...
function createArtifactId(value) {
	return `${artifactDomain}-${slug(typeof value.artifactType === "string" ? value.artifactType : "artifact")}-${contentHash(value).slice(0, 12)}`;
}
...
function attachArtifactMetadata(value, options = {}) {
	...
	if (options.staleAfterDays !== void 0 && !record.staleAfter && validDate(generatedAt)) result.staleAfter = addDays(generatedAt, options.staleAfterDays);
	const serialized = JSON.parse(JSON.stringify(result));
	serialized.contentHash = integrityHash(serialized);
	return serialized;
}
function reviewArtifact(value, expectedType) {
	const issues = [];
	const warnings = [];
	...
	else if (Date.parse(record.generatedAt) > Date.now() + 3e5) issues.push("generatedAt 不能是未来时间。");
	if (record.staleAfter !== void 0 && !validDate(record.staleAfter)) issues.push("staleAfter 必须是有效 ISO 日期或时间。");
	...
	} else if (record.hashVersion !== void 0) issues.push("不支持的 hashVersion。");
	else warnings.push("旧版或缺少内容校验版本；请由来源插件重新生成，不能自动视为已验证。");
...
async function appendArtifactAudit(fs, root, event, signal) {
	const path = join(root, ".dsh-product-audit.jsonl");
...
const handoffRoutes = {
	...
	"commercial-handoff": {
		...
		"lists": ["offers", "requiredApprovals"],
		"mode": "direct"
	},
...
//#endregion
//#region src/handoff-receive.ts
const handoffParameters = {
	artifactJson: {
		type: "string",
		required: true,
		description: "Original JSON artifact or successful result envelope; preserve its checksum."
	},
	initiativeId: {
		type: "string",
		required: true,
		description: "Stable ID shared by all receipts for this business initiative."
	},
	owner: {
		type: "string",
		required: true,
		description: "Human responsible for the next action."
	},
	action: {
		type: "string",
		required: true,
		description: "Concrete next action, not a claim of completion."
	},
	dueDate: {
		type: "string",
		required: true,
		description: "ISO date or timestamp for the action deadline."
	}
};
...
function receiveHandoff(value, options) {
	...
	const missing = [];
	const sourceType$1 = text$1(source.artifactType);
	const route = Object.hasOwn(handoffRoutes, sourceType$1) ? handoffRoutes[sourceType$1] : void 0;
	if (isEnvelope && envelope.ok !== true) issues.push("来源工具未成功，不能消费其 data。");
	if (!route) issues.push(`本插件不接收 ${sourceType$1 || "unknown"}，请按职责交接。`);
	if (route) {
		...
		for (const field of route.text) if (!text$1(source[field])) missing.push(field);
	...
	for (const field of [
		...
		"owner",
		"action"
	]) if (!options[field].trim()) missing.push(field);
	...
	if ([
		...
		"stale",
		"rejected"
	].includes(text$1(source.status)) || ["hold", "stop"].includes(text$1(source.decision))) issues.push("上游工件要求停止或重新审查，不能向下推进。");
	if ([
		"partial",
		"draft",
		"open",
		"implemented",
		"accepted"
	].includes(text$1(source.status))) missing.push("上游尚未完成验证");
	if (review.status === "stale") issues.push("来源已过期，必须回到生产插件重新核验。");
	if (sourceType$1 === "opportunity-handoff" && Object.keys(asRecord(source.validationExperiment)).length === 0) missing.push("validationExperiment");
	if (sourceType$1 === "product-sales-handoff" && !["proceed", "scale"].includes(text$1(source.productDecision))) missing.push("productDecision");
	if (sourceType$1 === "product-feedback-closure" && source.status !== "verified") missing.push("反馈尚未 verified");
	if (sourceType$1 === "sales-feedback-handoff") {
		if (!["dsh-growth"].includes(artifactDomain) && (!Array.isArray(source.feedback) || !source.feedback.length || !source.feedback.every((value$1) => {
			const row = asRecord(value$1);
			return typeof row.reason === "string" && !!row.reason.trim() && typeof row.count === "number" && Number.isInteger(row.count) && row.count > 0;
	...
	const sourceWarnings = [source.warnings, envelope.warnings].flatMap((items) => Array.isArray(items) ? items.filter((item) => typeof item === "string" && !!item.trim()) : []);
	...
	const context = Object.fromEntries(contextKeys.filter((key) => source[key] !== void 0).map((key) => [key, source[key]]));
	const nextActions = status === "accepted" ? [
		`${options.owner.trim()}：在 ${options.dueDate} 前${options.action.trim()}。`,
		`下一工具：${route?.nextTool}。`,
		route?.purpose ?? ""
	] : [
		...issues,
		...missing.map((field) => `补齐或核验：${field}。`),
		"修正来源并重新接收；不要修改回执状态冒充完成。"
	];
	return {
		artifactType: "handoff-receipt",
		generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
		initiativeId: options.initiativeId.trim(),
		receivedBy: artifactDomain,
		mode: route?.mode ?? "direct",
		status,
		sourceArtifactId: text$1(source.artifactId),
		sourceArtifactType: sourceType$1,
		sourceContentHash: text$1(source.contentHash),
		sourceProducer: text$1(source.handoffFrom),
		sourceStatus: text$1(source.status),
		...
		issues,
		missing,
		warnings: [...new Set([...warnings, ...sourceWarnings])],
...
//#endregion
//#region src/feedback-closure.ts
function closeProductFeedback(value, input) {
	const envelope = asRecord(value);
	const source = asRecord(Object.hasOwn(envelope, "data") ? envelope.data : value);
	const review = reviewArtifact(source);
	const issues = [...review.issues];
	const warnings = [...review.warnings];
	const sourceType$1 = source.artifactType;
	if (Object.hasOwn(envelope, "data") && envelope.ok !== true) issues.push("来源工具调用失败。");
	if (!["sales-feedback-handoff", "beta-feedback-import"].includes(String(sourceType$1))) issues.push("只接收标准销售或 Beta 反馈。");
	if (sourceType$1 === "sales-feedback-handoff" && (source.handoffFrom !== "dsh-sales" || source.handoffTo !== "dsh-product")) issues.push("销售反馈必须由 dsh-sales 发给 dsh-product。");
	if (sourceType$1 === "sales-feedback-handoff" && (!Array.isArray(source.feedback) || !source.feedback.length)) issues.push("没有实际销售反馈，不能标记处理完成。");
	if (sourceType$1 === "beta-feedback-import" && (!Array.isArray(source.records) || !source.records.length)) issues.push("没有实际 Beta 反馈，不能标记处理完成。");
	...
	if (![
		...
		"implemented",
		"verified"
	].includes(input.status)) issues.push("未知反馈状态。");
	const needsEvidence = [
		"rejected",
		"implemented",
		"verified"
	...
	const verification = input.verificationJson ? asRecord(JSON.parse(input.verificationJson)) : {};
	const verified = typeof verification.method === "string" && !!verification.method.trim() && typeof verification.source === "string" && !!verification.source.trim() && validDate(verification.observedAt) && Date.parse(verification.observedAt) <= Date.now() && validDate(source.generatedAt) && Date.parse(verification.observedAt) >= Date.parse(source.generatedAt) && verification.result === "passed";
	if (input.status === "verified" && !verified) warnings.push("verified 需要反馈生成后完成的复验：method、source、observedAt、result=passed。");
	const status = issues.length ? "blocked" : review.status !== "ready" || needsEvidence && !input.evidence?.trim() || input.status === "verified" && !verified ? "partial" : input.status;
	const nextActions = status === "verified" ? ["向原反馈方回传此工件及 sourceArtifactId，调用对应 handoff_receive 接收回执，再用后续数据复盘。", "复验通过不代表收入、留存或成交改善；业务效果仍需单独测量。"] : [
		...issues,
		...warnings,
		"由 owner 完成动作并提供实施/复验证据，再更新状态。"
	];
	return {
		artifactType: "product-feedback-closure",
		generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
		initiativeId: input.initiativeId.trim(),
		handoffFrom: "dsh-product",
		handoffTo: sourceType$1 === "sales-feedback-handoff" ? "dsh-sales" : "dsh-growth",
		...
		verification: Object.keys(verification).length ? verification : void 0,
		verificationBasis: "user-supplied-evidence",
		businessImpact: "not-measured",
		issues,
		warnings,
		nextActions
	};
...
function renderResult(value, maxChars) {
	const text$2 = JSON.stringify(value, null, 2);
	return [{
		type: "text",
		text: text$2.length > maxChars ? `${text$2.slice(0, maxChars)}\n... result truncated by dsh-product; use a narrower source or scope ...` : text$2
	}];
}
...
function wrapResult(value, options = {}) {
	...
	return resultEnvelope({
		data: jsonValue(attachArtifactMetadata(value, { staleAfterDays: 60 })),
		warnings,
		assumptions: options.assumptions,
		lineage: options.lineage,
		nextActions: options.nextActions
...
function registerProductTools(ctx, config, fs, web) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			publicFacts: {
				type: "string",
				required: true,
				description: "Only facts explicitly approved for public use; JSON string array."
			},
			evidence: {
				type: "string",
				required: true,
				description: "Traceable evidence references; JSON string array. Do not include private document contents."
			},
			claimBoundaries: {
				type: "string",
				required: true,
				description: "Prohibited or unproven claims; JSON string array."
			},
			targetMetric: {
				type: "string",
				required: true,
				description: "Downstream observable metric, not a ranking promise."
			},
			source: {
				type: "string",
				required: true
		...
		async execute(args) {
			...
			for (const field of [
				...
				"targetMetric",
				"source"
			]) if (!args[field].trim()) throw new Error(field + " must not be blank");
			return wrapResult({
				artifactType: "product-discoverability-handoff",
				generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
				handoffFrom: "dsh-product",
				handoffTo: "dsh-geo",
				status: "ready",
				initiativeId: args.initiativeId.trim(),
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			feedbackJson: {
				type: "string",
				required: true,
				description: "JSON returned by sales_feedback_handoff or product_beta_feedback_import."
			},
			initiativeId: {
				type: "string",
				required: true,
				description: "Same initiative ID as the incoming receipt."
			},
			verificationJson: {
				type: "string",
				description: "For verified: JSON object with method, source, observedAt ISO timestamp and result=passed. Recheck must occur after feedback generation."
			},
			action: {
				type: "string",
				required: true,
				description: "Concrete product action or decision."
			},
			owner: {
				type: "string",
				required: true,
			...
			dueDate: {
				type: "string",
				required: true,
				description: "ISO due date for the owned action."
			},
			status: {
				type: "string",
				required: true,
				...
				description: "Explicit closure status."
			},
			evidence: {
				type: "string",
				description: "Evidence for implementation or verification."
			}
		},
		output: productOutput(config.maxResultChars),
		async execute(args) {
			return wrapResult(closeProductFeedback(JSON.parse(args.feedbackJson), args));
		}
	}));
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const receipt = receiveHandoff(JSON.parse(args.artifactJson), args);
	...
	ctx.tools.register(defineTool({
		...
		async execute(args) {
			let value;
			try {
				value = JSON.parse(args.artifactJson);
			} catch (error) {
				throw new Error(`artifactJson must be valid JSON: ${error instanceof Error ? error.message : String(error)}`);
			}
			const review = reviewArtifact(typeof value === "object" && value !== null && "data" in value ? value.data : value, args.expectedType?.trim() || void 0);
			return wrapResult({
				artifactType: "product-artifact-review",
				generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
				...review
			}, { nextActions: review.nextActions });
		}
